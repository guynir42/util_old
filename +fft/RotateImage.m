...
function[Image] = RotateImage(Image, degrees)
    ...
    if size(Image,3)>1
        ...
        if isscalar(degrees)
            degrees = repmat(degrees, [size(Image,3),1]);
        elseif length(degrees)~=size(Image,3)
            error(['size mismatch: size(degrees)= ' num2str(size(degrees)) ' | size(Image)= ' num2str(size(Image))]);
        end
        ...
        for ii = 1:size(Image,3)
            ...
            I(:,:,ii) = RotateImage(Image(:,:,ii), degrees(ii));
